...
/**
 * A {@link ModelWeld} resolved against a concrete model: the source cube's face glued onto the target
 * cube's face. The weld seals a bending joint by pulling both welded faces onto a shared seam, so the
 * bend distributes across both cubes (the parent's face shears to meet the child's, the child's shears
 * back) instead of only the child deforming against a rigid parent.
 *
 * <p>The corners are the box's, not a drawn quad's ({@link ModelCube#faceCorners}), so a side that
 * isn't drawn welds all the same — the sides meeting at a joint usually aren't.</p>
 *
 * <p>Because the parent draws before the child, the rigid world poses of both faces can't be known in one
 * traversal — the renderer runs a capture pass first (it fills the captured state after
 * {@link #resetCapture}), then {@link #computeSeam}, then the draw pass snaps to {@link #seam}.</p>
 */
public class WeldBinding
{
    ...
    /** Why a weld can't resolve against the model — for the editor to say, instead of a silent no-op. */
    public enum Issue
    {
        /** A cube of the weld isn't in the model. */
        CUBE,

        /** Both cubes sit in one group — rigid, so there is no joint between them to seal. */
        SAME_GROUP
    }
     ...
     * a weld passing this always yields a binding, so the two can't drift apart.
     */
    public static Issue diagnose(Model model, ModelWeld weld)
    {
        ModelGroup source = model.findGroup(weld.sourceCube);
        ModelGroup target = model.findGroup(weld.targetCube);

        if (source == null || target == null || weld.sourceFace == null || weld.targetFace == null)
        {
            return Issue.CUBE;
        }

        return source == target ? Issue.SAME_GROUP : null;
    }
    ...
    public static WeldBinding resolve(Model model, ModelWeld weld)
    {
        ...
        ModelGroup sourceGroup = model.findGroup(weld.sourceCube);
        ModelGroup targetGroup = model.findGroup(weld.targetCube);
        ...
        return new WeldBinding(sourceGroup, targetGroup, weld, targetShare);
    }
    ...
    public static boolean hasActiveSeam(List<WeldBinding> welds, ModelGroup group)
    {
        ...
        {
            if ((weld.sourceGroup == group || weld.targetGroup == group) && weld.seamReady && !weld.identity)
            {
                return true;
            }
        }
    ...
    /**
     * Everything the seam needs that doesn't move with the pose, read off the rest pose: which corner of
     * the source face meets which corner of the target's, and the twist already between them. Decided HERE,
     * in the one pose where the modeler actually aligned them — matching by proximity in whatever animated
     * pose the first frame happens to catch fixes a bent/twisted mapping forever.
     */
    private WeldBinding(ModelGroup sourceGroup, ModelGroup targetGroup, ModelWeld weld, float targetShare)
    {
        this.sourceGroup = sourceGroup;
        this.targetGroup = targetGroup;
        this.sourceCube = weld.sourceCube;
        this.targetCube = weld.targetCube;
        this.sourceCorners = weld.sourceCube.faceCorners(weld.sourceFace);
        this.targetCorners = weld.targetCube.faceCorners(weld.targetFace);
        this.targetFaceNormal = new Vector3f(weld.targetFace.normal);
        this.sourceFaceNormal = new Vector3f(weld.sourceFace.normal);
        ...
        this.targetAxisExtent = axisExtent(weld.targetCube, weld.targetFace);
        this.sourceAxisExtent = axisExtent(weld.sourceCube, weld.sourceFace);
        ...
        Vector3f[] sourceRestWorld = transformCorners(restCubeMatrix(sourceGroupRest, weld.sourceCube), this.sourceCorners);
        Vector3f[] targetRestWorld = transformCorners(restCubeMatrix(targetGroupRest, weld.targetCube), this.targetCorners);
    ...
    private static int[] matchCorners(Vector3f[] sourceRest, Vector3f[] targetRest)
    {
        ...
        for (int[] permutation : PERMUTATIONS)
        {
            ...
            for (int r = 0; r < permutation.length; r++)
            {
    ...
    /**
     * How far the cube spans along the welded face's normal, in blocks: the box's side along that axis,
     * the inflate on both ends. Unsigned — a size typed or scaled below zero turns the box inside out, and
     * a band measured off a negative length would collapse the bend into a crease.
     */
    private static float axisExtent(ModelCube cube, CubeFace face)
    {
        Vector3f normal = face.normal;
        float side = Math.abs(normal.x) * cube.size.x + Math.abs(normal.y) * cube.size.y + Math.abs(normal.z) * cube.size.z;

        return Math.abs(side + 2F * cube.inflate) / 16F;
    }
